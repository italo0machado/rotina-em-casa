...
description: Use when working with Blossom Carousel Core, the framework-agnostic carousel engine, APIs, options, events, button controls, overscroll behavior, lifecycle, layout examples, optional Tailwind examples, accessibility or a11y guidance, and integration patterns.
---
...
The object returned by `Blossom(element)` is the Blossom instance. Use that instance for `init()`, `destroy()`, `prev()`, and `next()`.
...
Use the Blossom instance when the user needs previous and next buttons. Blossom exposes `prev()` and `next()` for programmatic navigation:
...
```html
<button onclick="blossom.prev({ align: 'center' })">Previous</button>
<button onclick="blossom.next({ align: 'center' })">Next</button>
```

Use `prev()` and `next()` on the Blossom instance rather than manually changing scroll positions. The `align` option accepts `"start" | "center" | "end"`.
...
- For custom controls, call `prev()` or `next()` on the Blossom instance returned by `Blossom(element)`, not on the raw DOM element.
...
Do not call `init()` more than once per instance, and do not call `prev()`, `next()`, or other methods after `destroy()`.
...
If custom previous or next buttons do nothing, check that the Blossom instance was created and initialized, and that handlers call `blossomCarousel.prev()` or `blossomCarousel.next()` on the instance.
